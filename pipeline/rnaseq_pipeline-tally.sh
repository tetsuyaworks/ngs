...
for sample_name in $samplename_list; do
    ...
    output_path=$OUTPUT_DIR/$sample_name.txt
    cutadapt -j $USE_CPU -a $ADAPTER -o $tmpdir/1.fq ${samplefile_dict[$sample_name,R1]}
    ...
    featureCounts -T $USE_CPU -t exon -g gene -a $GFF_PATH -o $tmpdir/1.tsv $tmpdir/1.bam

    sed -i '1d' $tmpdir/1.tsv
    sed -i -E 's/^([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)$/\1\t\6\t\7/g' $tmpdir/1.tsv
    sed -i '1c Geneid\tLength\tCount' $tmpdir/1.tsv
    sort -k 1 $tmpdir/1.tsv > $output_path
    rm -rf $tmpdir/*
done
